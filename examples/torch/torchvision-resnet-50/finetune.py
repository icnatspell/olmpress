...
# ///
"""Fine-tune a pruned torchvision ResNet50 student with optional knowledge distillation.

Preprocessing uses standard torchvision transforms throughout (Resize 256 →
...
Usage:
    uv run examples/torch/torchvision-resnet-50/finetune.py --model outputs/lamp_0.10/model/model.pt
...
def _eval_accuracy(model_path: str, num_samples: int, batch_size: int) -> float:
    ...
    Uses standard torchvision val transforms and evaluate library,
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="CE loss weight a in a*CE + (1-a)*t^2*KL (kd mode only)",
    )
