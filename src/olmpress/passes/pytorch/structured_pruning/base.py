...
class TorchPruningPass(Pass):
    ...
    def _run_hf(
        ...
        if hasattr(cfg, "num_channels"):
            # Vision model: pass as dict so torch-pruning calls model(**inputs).
            raw = getattr(cfg, "image_size", 224)
            img = raw if isinstance(raw, int) else raw[0]
            num_ch: int = cfg.num_channels  # type: ignore[assignment]
            example_tensor: torch.Tensor = torch.randn(1, num_ch, img, img)  # type: ignore[arg-type]
            example_inputs: Any = {"pixel_values": example_tensor}
            use_pixel_values = True
        else:
            example_tensor = torch.randint(0, int(cfg.vocab_size), list(config.example_input_shape))  # type: ignore[union-attr]
            example_inputs = example_tensor
            use_pixel_values = False

        ignored = resolve_ignored_layers(pt_model, config.ignored_layers or [])
        # HF models return ModelOutput objects; output_transform extracts a scalar
        ...
        # Wrap to a stable forward(tensor) -> logits interface for TorchScript.
        class _LogitsWrapper(torch.nn.Module):
            def __init__(self, model: torch.nn.Module, *, pixel_values: bool) -> None:
                super().__init__()
                self._m = model
                self._pv = pixel_values

            def forward(self, x: torch.Tensor) -> torch.Tensor:
                out = self._m(pixel_values=x) if self._pv else self._m(x)
                return out.logits if hasattr(out, "logits") else out[0]

        out_path = Path(output_model_path).with_suffix(".pt")
        out_path.parent.mkdir(parents=True, exist_ok=True)
        scripted = torch.jit.trace(
            _LogitsWrapper(pt_model, pixel_values=use_pixel_values), example_tensor
        )
